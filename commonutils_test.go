...
func TestUnmarshalPayload(t *testing.T) {
	...
	good, _ := proto.Marshal(&common.Payload{
		Data: []byte("payload"),
	})
...
func TestUnmarshalEnvelope(t *testing.T) {
	...
	good, _ := proto.Marshal(&common.Envelope{})
...
func TestUnmarshalBlock(t *testing.T) {
	var env *common.Block
	good, _ := proto.Marshal(&common.Block{})
...
func TestUnmarshalEnvelopeOfType(t *testing.T) {
	...
	_, err := UnmarshalEnvelopeOfType(env, common.HeaderType_CONFIG, nil)
	...
	env.Payload = payload
	_, err = UnmarshalEnvelopeOfType(env, common.HeaderType_CONFIG, &common.ConfigEnvelope{})
	require.Error(t, err, "Expected error for malformed payload data")

	chdr, _ = proto.Marshal(&common.ChannelHeader{
		Type: int32(common.HeaderType_CONFIG),
	})
	configEnv, _ := proto.Marshal(&common.ConfigEnvelope{})
	payload, _ = proto.Marshal(&common.Payload{
		Header: &common.Header{
			ChannelHeader: chdr,
		},
...
func TestUnmarshalChaincodeID(t *testing.T) {
	ccname := "mychaincode"
	ccversion := "myversion"
	ccidbytes, _ := proto.Marshal(&peer.ChaincodeID{
		Name:    ccname,
		Version: ccversion,
	})
...
func testPayload() *common.Payload {
	return &common.Payload{
		Header: MakePayloadHeader(
			MakeChannelHeader(common.HeaderType_MESSAGE, int32(1), "test", 0),
			MakeSignatureHeader([]byte("creator"), []byte("nonce"))),
		Data: []byte("test"),
	}
}

func testEnvelope() *common.Envelope {
	// No need to set the signature
	return &common.Envelope{Payload: MarshalOrPanic(testPayload())}
}

func testBlock() *common.Block {
...
func TestChannelHeader(t *testing.T) {
	makeEnvelope := func(payload *common.Payload) *common.Envelope {
		return &common.Envelope{
			Payload: MarshalOrPanic(payload),
		}
	}

	_, err := ChannelHeader(makeEnvelope(&common.Payload{
		Header: &common.Header{
	...
	_, err = ChannelHeader(&common.Envelope{})
	require.Error(t, err, "Payload was missing")
}

func TestIsConfigBlock(t *testing.T) {
	newBlock := func(env *common.Envelope) *common.Block {
		return &common.Block{
			Data: &common.BlockData{
	...
	newConfigEnv := func(envType int32) *common.Envelope {
		return &common.Envelope{
	...
	envType := int32(common.HeaderType_CONFIG)
	...
	envType = int32(common.HeaderType_ORDERER_TRANSACTION)
	env = newConfigEnv(envType)
	block = newBlock(env)
	...
	envType = int32(common.HeaderType_MESSAGE)
	env = newConfigEnv(envType)
	block = newBlock(env)
	...
	result = IsConfigBlock(&common.Block{
		Header:   nil,
		Data:     &common.BlockData{Data: [][]byte{{1, 2, 3, 4}, {1, 2, 3, 4}}},
		Metadata: nil,
	})
...
func TestEnvelopeToConfigUpdate(t *testing.T) {
	makeEnv := func(data []byte) *common.Envelope {
		return &common.Envelope{
			Payload: MarshalOrPanic(&common.Payload{
				Header: &common.Header{
					ChannelHeader: MarshalOrPanic(&common.ChannelHeader{
